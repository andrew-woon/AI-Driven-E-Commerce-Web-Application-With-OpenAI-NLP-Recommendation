...
def mycart(request):
    ...
    return render(request, 'cart_data.html', {'cart': cart,'cartproducts':cartproducts})
...
def checkoutdetails(request):
    ...
    cartproducts = cart.cartproduct_set.all().order_by('-id')
    return render(request, "delivery-info.html",{'profile': profile, 'address':address, 'order':order, 'cart': cart,'cartproducts':cartproducts})