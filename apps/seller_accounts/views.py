...
def profile(request):
    if check_is_seller(request):
        ...
        return redirect('apps.seller_accounts:signin')
    return render(request, "seller_profile.html", {'seller_user':user})
...
def company_details(request):
    if check_is_seller(request):
        ...
        messages.warning(request,'Login as Seller Account.')
        return redirect('apps.seller_accounts:signin')
    return render(request, "seller_cpdetail.html")
...
def address_info(request):
    if check_is_seller(request):
        ...
        messages.warning(request,'Login as Seller Account.')
        return redirect('apps.seller_accounts:signin')
    return render(request, "seller_address.html")
...
def add_single_product(request):
    if check_is_seller(request):
        if request.method == 'POST':
            ...
            product_gender = request.POST.get("product_gender")
            product_sub_cat = request.POST.get("product_sub_cat")
            product_articel_type = request.POST.get("product_articel_type")
            ...
            product = Product.objects.create(title=product_name,
                                            gender_cat=product_gender,
                                            sub_cat=product_sub_cat,
                                            articel_type=product_articel_type,
...
def editproduct(request, proid):
    ...
    return render(request, "update-product.html",{'product':product,'product_img_urls': product_img_urls})
...
def companyinfo(request):
    if check_is_seller(request):
        ...
        return redirect('apps.seller_accounts:signin')
    return render(request, "seller_cpinfo.html",{'company_info': company_info, 'address_info': addressinfo})
...
def companyproducts(request):
    if check_is_seller(request):
        ...
        return render(request, "products.html",{'sale':product_list,'sale_no_products':sale_products_len})
    else:
        return redirect('apps.seller_accounts:signin')
